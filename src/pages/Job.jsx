  ...
  return (
    <div className='flex flex-col gap-8 mt-5'>
      <div className='flex flex-col-reverse gap-6 md:flex-row justify-between items-center'>
        ...
        <img src={job?.company?.logo_url} className='h-12' alt={job?.title}></img>
      </div>
      ...
      {/* Render Applications */}
      {
        job?.recruiter_id !== user?.id && <ApplyJobDrawer job={job} user={user} fetchJob={fnJob} applied={job?.applications.find((ap)=> ap.candidate_id === user.id)}/>      // show apply to job button only when it is not a recruiter.
      }
      {
